...
const getNextTracks = async (
  ...
  const next_tracks_batch: RecommendationsResponse = await doWithRateLimiter(
    (params: RecommendationsRequest) => sdk.recommendations.get(params),
    [
      {
  ...
  const next_tracks = next_tracks_batch.tracks
    .filter(
      (t) =>
        !seed_ids.includes(t.id) &&
        !seed_artists.includes(t?.album?.artists[0]?.name) &&
        (allow_explicit || !t.explicit)
    )
    .map((t) => ({
      track: t,
      added_at: "", // Placeholder or actual value
    ...
    <>
      ...
      >
        {!session || session.status != "authenticated" ? (
          <Button color="success" onClick={() => signIn("spotify")}>
    ...
    (async () => {
      try {
        if (selected) {
          ...
          let item = await doWithRateLimiter(
    ...
    (async () => {
      try {
        if (selectedTrack) {
          ...
          let trackInfo = await doWithRateLimiter(
            (params: string) => sdk.tracks.audioFeatures(params),
                ...
                >
                  ...
                  <td>
                    <a href={playlist?.external_urls?.spotify} target="_blank">
                      <img src="/spotify_icon.png" style={{ height: "20px" }} />
                    </a>
                  </td>
                </tr>
              ))}
